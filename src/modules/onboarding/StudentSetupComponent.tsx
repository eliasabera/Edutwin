...
import { fetchOnboardingQuestions } from "../../../shared/services/onboarding-service";
...
const SUBJECT_LABELS: Record<SubjectName, string> = {
  biology: "Biology",
  chemistry: "Chemistry",
  physics: "Physics",
  math: "Math",
...
const resolveQuestionGrade = (value: string | null) => {
  const parsed = Number.parseInt(String(value || "").trim(), 10);
  if (!Number.isFinite(parsed)) return 8;
  return Math.max(8, parsed - 1);
};
...
export default function StudentSetupComponent() {
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    const questionGrade = resolveQuestionGrade(grade);

    const loadQuestions = async () => {
      setIsLoadingQuestions(true);
      setQuestionLoadError("");

      const fetched = await fetchOnboardingQuestions(questionGrade);
      if (!isMounted) return;
      ...
      if (!fetched.length) {
        setQuestionLoadError(
          `No onboarding questions found for grade ${questionGrade}.`,
        );
      }
  ...
  const startQuiz = () => {
    ...
    if (isLoadingQuestions) {
      Alert.alert("Loading", "Fetching onboarding questions...");
      return;
    }
    if (!questions.length) {
      Alert.alert("No questions", questionLoadError || "No questions found.");
      return;
    }
  ...
  const goToHome = async () => {
    if (!questions.length) {
      Alert.alert("No questions", "No onboarding questions available.");
      return;
    }
  ...
  if (currentStep === "quiz") {
    const currentQ = questions[qIndex];
    const progress = questions.length ? (qIndex + 1) / questions.length : 0;

    if (!currentQ) {
      return (
        <View style={[styles.container, { paddingTop: 60 }]}>
          <View style={styles.header}>
            <Text style={styles.progressLabel}>Loading questions...</Text>
          </View>
          <Text style={styles.subtitle}>{questionLoadError}</Text>
        </View>
      );
    }
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
          ...
          <View style={styles.badge}>
            <Text style={styles.badgeText}>
              {SUBJECT_LABELS[currentQ.subject] || currentQ.subject}
            </Text>
          </View>
    ...
    return (
      ...
      >
        ...
        <View
          style={[
            styles.resultCircle,
              { borderColor: isGood ? COLORS.primary : "#FFC107" },
          ]}
        >
...
const styles = StyleSheet.create({
  ...
  cardSelected: { borderColor: COLORS.primary, backgroundColor: "#E6F0FF" },
  cardText: { fontSize: 16, fontWeight: "600", color: COLORS.textLight },
  cardTextSelected: { color: COLORS.primary, fontWeight: "bold" },
